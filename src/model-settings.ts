...
type SavedSettings = { version: 1; baseUrl: string; model: string; apiKey: string };
...
export class ModelSettings {
  ...
  constructor(dataDir: string, initial: NodeJS.ProcessEnv) {
    ...
    try {
      ...
      this.apply({ version: 1, ...endpoint, apiKey: this.key(value.apiKey) });
  ...
  view(): ModelSettingsView {
    return { baseUrl: this.env.ANTHROPIC_BASE_URL?.trim() || "https://api.anthropic.com", model: this.env.ANTHROPIC_MODEL?.trim() || "", hasApiKey: !!this.env.ANTHROPIC_API_KEY?.trim(), source: this.saved ? "saved" : "environment" };
  }
  save(input: unknown): ModelSettingsView {
    ...
    const next: SavedSettings = { version: 1, ...endpoint, apiKey };
