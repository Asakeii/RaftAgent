...
export interface Message { id: string; channel: string; sender: string; text: string; at: string; mentions: string[]; seq?: number; internalFor?: string; legacyContext?: boolean; runId?: string; roomVersion?: number; basedOn?: number; delivery?: "streaming" | "interrupted"; }
...
export interface Run { id: string; agentId: string; inputId: string; channel?: string; sdkSessionId?: string; replyTarget?: string; status: "running" | "done" | "error" | "unknown"; at: string; }
...
export interface AppState { agents: Agent[]; rooms: Room[]; messages: Message[]; receipts: Receipt[]; drafts: Draft[]; tasks: Task[]; activities: Activity[]; inputs: Input[]; runs: Run[]; notices: Record<string, number>; contextVersion?: number; sharedInboxVersion?: number; sessions?: ConversationSession[]; sceneNotices?: Record<string, number>; requests: Record<string, { fingerprint: string; result: unknown }>; publishedSkills?: PublishedSkill[]; skillCatalog?: SharedSkill[]; skillCatalogVersion?: number; events: Audit[]; seq: number; }
