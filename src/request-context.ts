...
export function requestForMessage(s: AppState, message: Message): string | undefined {
  ...
  return s.messages.filter(m => m.channel === message.channel && m.sender === 'user' && !m.internalFor && (m.seq ?? 0) < (message.seq ?? 0)).at(-1)?.id;
}
export function requestForInput(s: AppState, channel: string, messageIds: string[] = []) {
  ...
  const userRequest = triggers.filter(m => m.sender === 'user' && !m.internalFor).at(-1);
...
export function requestState(s: AppState, agentId: string, requestId?: string) {
  const request = s.messages.find(m => m.id === requestId && m.sender === 'user' && !m.internalFor);
  if (!request) return undefined;
  const replies = s.messages.filter(m => m.channel === request.channel && m.sender === agentId && !m.internalFor && requestForMessage(s, m) === requestId);
...
export function inboxItem(s: AppState, agentId: string, m: Message) {
  return { id: m.id, sender: m.sender, at: m.at, mentioned: m.mentions.includes(agentId),
    source: m.sender === 'user' ? 'user_request' : m.internalFor ? 'delegation_result' : 'member_update',
