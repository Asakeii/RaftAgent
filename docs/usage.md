...
群聊共享一份公开历史，每个成员维护自己的待处理 inbox。正常模型正文由宿主在结束前校验并发布；不参与时调用 `raftctl room silence` 结束本轮。显式 @ 交接或跨群发送使用 `raftctl room send --mentions`。多名成员同时回复时，服务会检查它们依据的群版本：版本变化会拦截发布并将最新 inbox 返回当前 Agent，让它重新决定发送、修订或丢弃。群消息完整提交后显示，私聊保留流式展示；已发布的错误或不完整消息可以撤回。

右侧状态栏可以查看成员、添加成员、管理任务和处理草稿。任务提交后由用户核验产物并验收；“Agent 已结束运行”不等于“任务已完成”。

私聊与各群聊使用独立 SDK 会话，但这不代表信息完全隔离：群运行会获得该 Agent 的近期私聊背景，也可以按权限检索其他场景。同一个 Agent 的不同会话仍共享其工作目录。详情见[群聊上下文设计](progressive-room-context-design.md)和[共享 inbox 与发言规则](shared-group-inbox.md)。

### Agent 监测

点击侧栏“Agent 监测”或会话顶部“监测系统”，工作区会翻转到独立监测页。页面集中展示运行数量、授权等待、异常成员及最近运行，支持按私聊/群聊和成员查看会话详情与执行日志。运行时间线按日期分组，展开可查看模型响应、工具调用、授权与异常节点，以及相对开始时间和实际采集耗时；支持状态筛选和实时追加，也可跳转完整日志。

点击“返回工作区”或按 Esc 翻回，聊天会话和输入草稿保留。系统开启减少动态效果时会取消翻转动画。原右侧面板只管理成员、任务和草稿。
...
在 Agent 会话顶部的 **Skills** 面板选择该成员可用的能力。Agent 可以在工作目录中编写 Skill，通过本地 CLI 发布，并在发布成功后热加载。共享 Skill 的源码、发布版本及成员启用配置由应用统一管理，详见[共享 Skill 说明](shared-skills.md)。
...
代码更新后需要重启本地服务。内置 Skill 首次导入后由数据目录中的共享库维护，项目模板更新不会自动覆盖已导入的源码；已有安装通常需要同步相应 Skill 源码，再发布更新。已知的旧 requestId 指令会在启动时自动迁移，普通 CLI 调用无需模型手填 ID。

**当前有哪些限制？**

本项目仍处于开发阶段，适合本地小规模使用。目前最多 12 个 Agent、3 个并发运行；每轮限制为 16 个模型轮次和 2 美元 SDK 预算，连续运行 30 次后暂停，新用户消息会重置连续次数。可配置人民币输入、输出和缓存命中单价，按 SDK 用量生成运行成本；实际计费以服务商账单为准。成员是否发言由模型判断，不保证每次群消息都有固定数量的回复。
...
| `npx tsx tests/chat-streaming-smoke.ts` | 私聊流式、群聊协作与停止验证 |
...
开发前请阅读 [agent.md](../agent.md)，新增能力先核对 [Claude Agent SDK 官方文档](https://code.claude.com/docs/en/agent-sdk/overview)与已安装版本。实现细节见[实现说明](implementation.md)；[多 Agent 设计](multi-agent-design.md)中的候选方案和调研内容不代表已实现功能。
