...
try {
  ...
  await page.getByLabel('选择群内 Agent').selectOption(a.id);
  await page.getByRole('tab', { name: '会话详情', exact: true }).click();
  ...
  await page.getByLabel('选择群内 Agent').selectOption(b.id);
  ...
  await page.getByRole('tab', { name: '执行日志', exact: true }).click();
  await page.getByRole('button', { name: /执行任务 group-sage/ }).click();
  await page.getByText('运行配置与用量', { exact: true }).waitFor();
  await page.locator('.sidebar').getByRole('button', { name: /Atlas/ }).click();
  await page.getByRole('tab', { name: '会话详情', exact: true }).click();
