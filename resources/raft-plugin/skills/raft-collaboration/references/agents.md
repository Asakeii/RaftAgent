...
```bash
raftctl agent create --name '测试审查员' --system-prompt '你负责检查测试覆盖与边界情况。报告实际证据，不改动文件。' --task '阅读登录模块与测试，列出三个最重要的遗漏。' --json
```
...
- 创建和后续委派由 CLI 自动分配 requestId；通信失败先按回执 ID 查询，确认重试时才显式复用原 ID 和参数。
...
```bash
raftctl agent send --id CHILD_ID --task '进一步核验第二项问题，给出具体文件位置。' --json
```
