...
用 Bash 执行 `node <本 Skill 的目录>/scripts/count.mjs <目标文件绝对路径>`。
本 Skill 的目录使用 Skill 加载结果给出的 base directory，不使用草稿目录。
...
```bash
raftctl skill publish --source ./skill-drafts/count-lines --request-id publish-count-001 --json
```
