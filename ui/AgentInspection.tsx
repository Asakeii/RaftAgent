...
export function AgentTraces({ agent, api, selectedRun, selectRun, onAgent }: { agent: Agent; api: Api; selectedRun: string; selectRun: (id: string) => void; onAgent: (id: string, runId: string) => void }) {
  const [before, setBefore] = useState('');
  const { data, error, reload } = useLive<TraceList>(api, `agents/${agent.id}/traces${before ? `?before=${encodeURIComponent(before)}` : ''}`);
  const active = data?.runs.find(r => r.status === 'running');
  const reason = data?.agentStatus === 'running' ? active?.phase || 'Agent 正在执行，返回最新列表查看当前运行。' : !data?.ready ? '未配置 API Key，消息等待执行。' : data.agentStatus === 'stopped' ? 'Agent 已停止，消息只排队；点击继续后执行。' : data.agentStatus === 'error' ? '上轮执行失败；处理问题后点击继续。' : active ? active.phase : data?.pending ? '等待调度，有空闲执行名额后开始。' : '当前没有待执行输入。';
  return <section className="inspection" aria-label="执行日志">
    ...
    {selectedRun ? <TraceRecord key={selectedRun} api={api} agentId={agent.id} runId={selectedRun} back={() => selectRun('')} onAgent={onAgent} /> : <>
...
function TraceRecord({ api, agentId, runId, back, onAgent }: { api: Api; agentId: string; runId: string; back: () => void; onAgent: (id: string, runId: string) => void }) {
  ...
  const load = async () => {
    ...
    try { const next = await api(`agents/${agentId}/traces/${runId}?after=${cursor.current}`) as TraceDetail;
  ...
  useEffect(() => { alive.current = true; void load(); const timer = setInterval(() => void load(), 2000); return () => { alive.current = false; clearInterval(timer); }; }, [agentId, runId]);
  ...
  return <div className="trace-record"><button className="back-link" onClick={back}>← 返回执行列表</button><ErrorNotice text={error || data?.warning || ''} />
    {run && <><div className="trace-title"><span className={`run-badge ${run.status}`}>{states[run.status]}</span><code>{run.id}</code></div><p className="trace-prompt">{run.prompt}</p>
      <div className="trace-metrics"><div><small>执行总耗时</small><strong>{duration(run.durationMs)}</strong></div><div><small>SDK API 累计耗时</small><strong>{duration(run.apiDurationMs)}</strong></div><div><small>SDK turns</small><strong>{run.turns ?? '—'}</strong></div><div><small>SDK 估算费用</small><strong>{run.estimatedCostUsd === undefined ? '—' : `$${run.estimatedCostUsd.toFixed(4)}`}</strong></div></div>
      <details className="trace-context"><summary>运行配置与用量</summary><dl><dt>模型</dt><dd>{run.model}</dd><dt>API 地址</dt><dd>{run.baseUrl}</dd><dt>会话</dt><dd>{run.sessionId || '未建立'}</dd><dt>开始</dt><dd>{time(run.startedAt)}</dd><dt>结束</dt><dd>{run.endedAt ? time(run.endedAt) : '未记录'}</dd><dt>来源</dt><dd>{run.kind} · {run.channel}</dd><dt>Trace</dt><dd>{run.traceId}</dd></dl><pre>{JSON.stringify(run.usage ?? {}, null, 2)}</pre><p>费用来自 SDK 估算，第三方模型不一定适用，以服务商账单为准。</p></details>
      {run.error && <ErrorNotice text={run.error} />}
      {!!data?.related.filter(r => r.id !== run.id).length && <div className="trace-related"><span>关联执行</span>{data?.related.filter(r => r.id !== run.id).map(r => <button key={r.id} onClick={() => onAgent(r.agentId, r.id)}>{short(r.agentId)} / {short(r.id)} · {states[r.status]} ↗</button>)}</div>}
    </>}
